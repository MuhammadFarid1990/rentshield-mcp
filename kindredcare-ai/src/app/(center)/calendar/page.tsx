...
export default async function CenterCalendarPage() {
  ...
  const seniorIds = (links ?? []).map((l) => (l.senior as { id: string })?.id).filter(Boolean);
  const seniorMap: Record<string, string> = Object.fromEntries(
    (links ?? []).map((l) => {
      const s = l.senior as { id: string; preferred_name?: string; user?: { full_name: string } };
      return [s.id, s.preferred_name ?? s.user?.full_name ?? "Senior"];
    }),
  );
